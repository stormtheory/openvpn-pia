...
CHOICE_LOCATIONS = pystray.Menu(Item("US", US_LOCATIONS), Item("Switzerland", locate_switzerland), Item("Netherlands", locate_netherlands), Item("UK London", locate_london))
        ...
        try:
            if str(ReturnCode) == '0':
                if service_state != 'online':
                    ...
                    systray.menu = pystray.Menu(Item("Turn Off", vpn_off), Item(LOCATION, btn_return), Item("Exit", exit_app))
                    SLEEP = 2
                    if os.geteuid() != 0:
                        time.sleep(6)
