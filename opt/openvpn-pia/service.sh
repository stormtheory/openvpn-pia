...
DEFAULT_LOCATION=us_missouri.ovpn
...
OPENVPN_CONFIG=/etc/openvpn/pia.conf
...
echo "LOAD: $LOAD"
	if [ -z "$LOAD" ];then
		...
		if [ ! -f $OPENVPN_CONFIG ];then
			ln -s $PIA_CONFIG_DIR/$LOAD $OPENVPN_CONFIG
		fi
...
if [ ! -f $OPENVPN_CONFIG ];then
	...
	sed -i "/auth-user-pass/c\\auth-user-pass $LOGIN_CREDS_FILE" $PIA_CONFIG_DIR/$LOAD
fi
}
